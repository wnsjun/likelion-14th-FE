...
const ButtonApply = ({ type, className, onClick }) => {
  ...
  */
  ...
  // 🔗 구글 폼 주소
  const GOOGLE_FORM_URL = "https://docs.google.com/forms/d/1wU1jXZtfOoAu_ksIwLwjeKB9eFCcHRAvPl04uhejgeo/viewform";
  ...
  const isOrangeStyle = isOngoing || (isAfter && (type === 'long_title' || type === 'long_footer'));

  const baseStyle = "flex justify-center items-center whitespace-nowrap rounded-full border-[1.5px] transition-colors duration-300";
  
  // 스타일 정의
  const orangeClasses = "bg-orange-01 hover:bg-orange-01-hover cursor-pointer border-orange-04 text-white"; // border 색상은 타입별 미세조정 필요시 덮어씌움
  const grayClasses = "bg-[#606060] border-[#606060] text-gray-04 cursor-pointer";

  // 타입별 크기/폰트 설정
  const long_title = 'w-fit pl-[32px] pr-[24px] py-[16px] title-20-bold';
  const short = 'w-fit px-[16px] py-[8px] body-18-semibold';
  const long_footer = 'pl-[32px] pr-[24px] py-[16px] title-20-bold';
